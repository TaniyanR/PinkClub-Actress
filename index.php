...
$title = $page > 1 ? '女優ピックアップ ' . $page . 'ページ目' : 'トップ';
$pageDescription = 'FANZAの女優・しろうと女優を写真から探せる女優専門サイト。女優プロフィールと出演作品をチェックできます。';
...
<style>
.pca-home-head{display:flex;align-items:end;justify-content:space-between;gap:16px;margin:0 0 16px}.pca-home-head h1{margin:0;font-size:28px}.pca-home-head p{margin:4px 0 0;color:#667085}.pca-home-count{font-size:13px;color:#667085;white-space:nowrap}.pca-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:16px}.pca-card{min-width:0;border:1px solid #e4e7ec;border-radius:10px;background:#fff;overflow:hidden;box-shadow:0 1px 2px rgba(16,24,40,.04)}.pca-card__link{display:block;color:inherit;text-decoration:none}.pca-card__image{display:block;width:100%;aspect-ratio:4/5;object-fit:cover;background:#f2f4f7}.pca-card__name{display:block;padding:10px 8px;text-align:center;font-size:14px;font-weight:700;line-height:1.45;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pca-card__tag{display:block;margin:-5px 8px 9px;text-align:center;font-size:11px;color:#667085}.pca-pagination{display:flex;justify-content:center;align-items:center;gap:6px;flex-wrap:wrap;margin:28px 0 8px}.pca-pagination a,.pca-pagination span{display:inline-flex;min-width:38px;height:38px;align-items:center;justify-content:center;padding:0 10px;border:1px solid #d0d5dd;border-radius:7px;text-decoration:none}.pca-pagination .is-current{background:#1d2939;color:#fff;border-color:#1d2939;font-weight:700}@media(max-width:1100px){.pca-grid{grid-template-columns:repeat(4,minmax(0,1fr))}}@media(max-width:720px){.pca-home-head{display:block}.pca-home-count{margin-top:6px}.pca-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.pca-card__name{font-size:13px}}
</style>

<section class="pca-home-head">
  <div>
    <h1>女優ピックアップ</h1>
    <p>女優としろうと女優をランダムに掲載しています。</p>
  </div>
  <div class="pca-home-count">全 <?= e(number_format($total)) ?> 名 / 1ページ120名</div>
</section>

<?php if ($actresses !== []): ?>
  <section class="pca-grid" aria-label="女優一覧">
    <?php foreach ($actresses as $actress): ?>
      ...
      <article class="pca-card">
        <a class="pca-card__link" href="<?= e($detailUrl) ?>">
          ...
          <span class="pca-card__tag"><?= $isAmateur ? 'しろうと女優' : '女優' ?></span>
        </a>
      </article>
    <?php endforeach; ?>
  </section>
<?php else: ?>
  <?php pcf_render_empty('女優データがまだありません。管理画面から女優API・商品APIの同期を実行してください。'); ?>
<?php endif; ?>
