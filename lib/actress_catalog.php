...
{
    $score = ((int)($row['_has_product'] ?? 0)) * 10000;
    if (pca_actress_image($row) !== '') $score += 100;
    foreach (['ruby', 'birthday', 'prefectures'] as $key) {
        if (trim((string)($row[$key] ?? '')) !== '') $score += 10;
    }
    return $score;
}
...
function pca_dedupe_normal_actress_rows(array $rows): array
{
    ...
    $result = array_map(static function (array $entry): array {
        $row = $entry['row'];
        unset($row['_has_product']);
        return $row;
    }, array_values($best));
...
{
    $groups = [];
    foreach (['あ', 'か', 'さ', 'た', 'な', 'は', 'ま', 'や', 'ら', 'わ', 'A-Z', '#'] as $key) $groups[$key] = [];
    foreach ($rows as $row) {
        if (!is_array($row)) continue;
